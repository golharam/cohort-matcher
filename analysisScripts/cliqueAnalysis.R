...
snp_threshold <- 150
snp_cutoff <- 0.7
paste("Using SNP Threshold:", snp_threshold, sep=" ")
paste("Using SNP Cut-off:", snp_cutoff, sep=" ")
...
png("snpdistr.png")
plot(cm.sorted$Fraction_Match,col=cm.sorted$color,main="SNP overlap, red is <100 SNPs and orange <150 SNPs")
...
cm.sorted$color[cm.sorted$SNPs_Compared<100] <- 'red'
cm.sorted$color[cm.sorted$SNPs_Compared<150&cm.sorted$SNPs_Compared>99] <- 'orange'
plot(cm.sorted$weighted,col=cm.sorted$color,main="SNP overlap, red is <100 SNPs and orange <150 SNPs")
abline(0.7,0,col="red")
abline(0.6,0,col="orange")
dev.off()

# Filter, should be args?
#cm.graph<-cm[cm$Fraction_Match>snp_cutoff & cm$SNPs_Compared>snp_threshold,c('Sample1','Sample2')]
cm.graph <- cm[cm$weighted>4,c('Sample1','Sample2')]

# Get the graph
cm.g <- graph_from_edgelist(as.matrix(cm.graph),directed=FALSE)
# Look only at the cliques with size >2 and do not report sub-cliques
