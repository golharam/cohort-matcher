...
for (sample in sample_to_subject$sample) {
  writeLines(paste("Checking", sample, sep=" "))
  ...
  if (nrow(sample_matches) > 0) {
    for (row_index in 1:nrow(sample_matches)) {
      ...
      writeLines(paste("    - ", matched_sample, sep=" "))
      ...
      matches <- rbind(matches, data.frame(from_sample=sample, from_subject=subject1, to_sample=matched_sample, to_subject=subject2, swap=swap))
    }
  }
}
...
# matches contains duplicate rows of (from_sample, to_sample) and (to_sample, from_sample).   We need to remove these duplicates
# before plotting, https://stackoverflow.com/questions/25297812/pair-wise-duplicate-removal-from-dataframe
cols = c(1, 2, 3, 4)
newdf = matches[,cols]
...
# Put a small gap between each subject
